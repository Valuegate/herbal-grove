...
function optionButtonClass(active: boolean, darkMode: boolean) {
  if (active) return "border-green-700 bg-green-700 text-white";
  return darkMode
...
export default function ConsultantBooking({ params }: Props) {
  const { id } = use(params);
  ...
  const upcomingDates = useMemo(() => {
    ...
    for (let i = 0; i < SLOT_LOOKAHEAD_DAYS; i++) {
                ...
                <div className="flex gap-3">
                  {DURATIONS.map((minutes) => (
                    <button
                      ...
                      className={`rounded-xl border px-5 py-3 text-sm font-semibold transition ${optionButtonClass(
                        duration === minutes,
                        darkMode
                      )}`}
                    >
              ...
              <div className="flex gap-2 overflow-x-auto pb-1">
                {upcomingDates.map(({ dateKey, date }) => (
                  <button
                    ...
                    className={`shrink-0 rounded-xl border px-4 py-3 text-center text-sm font-semibold transition ${optionButtonClass(
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
                  {activeDate.times.map((time) => (
                    <button
                      ...
                      className={`inline-flex items-center justify-center gap-2 rounded-xl border px-4 py-3 text-sm font-semibold transition ${optionButtonClass(
                        selectedTime === time,
                        darkMode
                      )}`}
                    >
