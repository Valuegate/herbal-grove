...
export default function ConsultantSidebar({
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={` fixed top-0 bottom-0 left-0 z-50 transition-all duration-300 ease-in-out lg:translate-x-0 lg:static lg:flex lg:flex-col lg:h-screen lg:shrink-0 ${collapsed ? "w-20" : "w-72"} ${isOpen ? "translate-x-0" : "-translate-x-full"} ${darkMode ? "bg-[#1E1E1E]" : "bg-white"}
        `}
      >
        {/* Header */}
        <div
            ...
            return (
              <Link
                ...
                className={` flex items-center ${collapsed ? "justify-center px-0" : "gap-4 px-4"} py-3.5 rounded-xl text-sm font-semibold transition-all duration-200 group
                  ${
                    active
                      ? darkMode ? "bg-emerald-500/10 text-emerald-400" : "bg-emerald-50 text-[#2b7a2d]"
                      : darkMode ? "text-emerald-400" : "text-[#2b7a2d]"
                  }
                `}
              >
